...
net = caffe.Net('/local2/home/tong/fashionRecommendation/models/fashionNet_9/fashion_deploy_9.prototxt', caffe.TEST)

# solver
solver = caffe.SGDSolver('/local2/home/tong/fashionRecommendation/models/fashionNet_9/training_record/t9.1_finetune_train_val/solver_prototxt/fashion_solver_9_k/fashion_solver_9_'+tUID+'.prototxt')

# caffemodel
caffemodel_path = '/local2/home/tong/fashionRecommendation/models/fashionNet_9/training_record/t9.1_train_val/fashion_params_9_'+cID+'.caffemodel'
solver.net.copy_from(caffemodel_path)
solver.test_nets[0].copy_from(caffemodel_path)

# results folder
recordDir = '/local2/home/tong/fashionRecommendation/models/fashionNet_9/training_record/t9.1_finetune_train_val/training_script/results/'
recordDir_data = recordDir+'data/'+test_fd+'/'

if 'results' not in os.listdir('/local2/home/tong/fashionRecommendation/models/fashionNet_9/training_record/t9.1_finetune_train_val/training_script/'):
...
for i in range (start_iter,end_iter+1):
    ...
    if (((i>=0) and (i%val_interval==0)) or (i==end_iter)):
        ...
        mean_NDCG_all = []
        mean_NDCG_all_idx = []
        for m in range(0,len(ndcg)):
            if (m%4==0):
                mean_NDCG_all_idx.append(int(ndcg[m].strip('\r\n').split(' ')[0]))
                mean_NDCG_all.append(float(ndcg[m].strip('\r\n').split(' ')[1]))
        fig = plt.figure()
        ax_left = fig.add_subplot(111)
        ax_left.plot(mean_NDCG_all_idx, mean_NDCG_all, '--r')
        lines_left, labels_left = ax_left.get_legend_handles_labels()   
        ax_left.legend(lines_left, labels_left, loc=0)
        ax_left.grid()
        ax_left.set_xlabel("Training Iterations Done(n)")
        ax_left.set_ylabel("mean_NDCG")
        ax_left.set_title("mean_NDCG during training of User_{}".format(tUID))
        plt.savefig(recordDir_data+'mean_NDCG.png', bbox_inches='tight')
        plt.close('all')        

        # top_10_posi.png
        top_10_posi = []
        top_10_posi_idx = []
        for t in range(0,len(ndcg)):
            if (t%4==1):
                top_10_posi_idx.append(int(ndcg[t].strip('\r\n').split(' ')[0]))
                top_10_posi_count = 0
                for c in range(0,10):
                    if (1 == int(float(ndcg[t].strip('\r\n').split(' ')[c+1]))):
                        top_10_posi_count += 1
                top_10_posi.append(top_10_posi_count)
        fig = plt.figure()
        ax_left = fig.add_subplot(111)
        ax_left.plot(top_10_posi_idx, top_10_posi, '--r')
        lines_left, labels_left = ax_left.get_legend_handles_labels()   
        ax_left.legend(lines_left, labels_left, loc=0)
        ax_left.grid()
        ax_left.set_xlabel("Training Iterations Done(n)")
        ax_left.set_ylabel("Top_10_posi_num")
        ax_left.set_title("Top_10_posi_num during training of User_{}".format(tUID))
        plt.savefig(recordDir_data+'top_10_posi.png', bbox_inches='tight')
        plt.close('all')   

        if (i==end_iter):
            initial_mNDCG_id = int(ndcg[0].strip('\r\n').split(' ')[0])
            initial_mNDCG = float(ndcg[0].strip('\r\n').split(' ')[1])
            optimal_mNDCG_id =  optimal_mNDCG_id
            optimal_mNDCG = max_mean_ndcg
            last_mNDCG_id = int(ndcg[-4].strip('\r\n').split(' ')[0])
