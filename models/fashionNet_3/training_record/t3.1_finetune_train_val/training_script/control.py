...
# results folder
recordDir = '/local2/home/tong/fashionRecommendation/models/fashionNet_3/training_record/t3.1_finetune_train_val/training_script/results/'
recordDir_data = recordDir+'data/U_'+uID+'/'

if 'results' not in os.listdir('/local2/home/tong/fashionRecommendation/models/fashionNet_3/training_record/t3.1_finetune_train_val/training_script/'):
...
# solver
solver = caffe.SGDSolver('/local2/home/tong/fashionRecommendation/models/fashionNet_3/training_record/t3.1_finetune_train_val/solver_prototxt/fashion_solver_3_k/fashion_solver_3_'+uID+'.prototxt')
    ...
    for i in range (start_iter,end_iter[c]+1):
        ...
        if (((i>=start_val_idx) and (i%val_interval==0)) or (i==end_iter[c])):
            ...
            if (c==0):
                ...
                net.save(recordDir_data+'fashion_params_3_'+str(i)+'.caffemodel') 
                ...
                for pr in params:
                    target_params[pr][0][...] = source_params[pr][0] #weights
                    target_params[pr][1][...] = source_params[pr][1] #bias
                net.save(recordDir_data+'cmp_fashion_params_3_'+str(i)+'.caffemodel')
    ...
    for i in range (start_val_idx,end_iter[c]+1):
        if ((i%val_interval==0) or (i==end_iter[c])):
            if (optimal_mNDCG_id != i):
                if (c==0):
                    os.system('rm '+recordDir_data+'fashion_params_3_'+str(i)+'.caffemodel')
                else:
                    os.system('rm '+recordDir_data+'cmp_fashion_params_3_'+str(i)+'.caffemodel')
